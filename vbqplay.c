...
Command line tool to play VBQ and QOA files

Requires:
	-"sokol_audio.h" (https://github.com/floooh/sokol/blob/master/sokol_audio.h)

Compile with: 
	gcc vbqplay.c -std=gnu99 -lasound -pthread -O3 -o vbqplay
...
qoaplay_desc *qoaplay_open(const char *path) {
	...
	qoa_desc qoa;
	unsigned int first_frame_pos = vbq_decode_header(header, QOA_MIN_FILESIZE, &qoa);
	if (!first_frame_pos) {
		return NULL;
	}
...
unsigned int qoaplay_decode_frame(qoaplay_desc *qp) {
	long pos = ftell(qp->file);
	qp->buffer_len = fread(qp->buffer, 1, qoa_max_frame_size(&qp->info), qp->file);

	unsigned int frame_len;
	long bytes_read = vbq_decode_frame(qp->buffer, qp->buffer_len, &qp->info, qp->sample_data, &frame_len);
    fseek(qp->file, bytes_read + pos, SEEK_SET);
